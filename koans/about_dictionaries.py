...
#Used Websit to review the following documentation on defining dictionaries 
...
#Used documentation for the following question below:
#https://www.geeksforgeeks.org/python-test-if-all-values-are-same-in-dictionary/
    ...
    def test_accessing_dictionaries(self):
        babel_fish = { 'one': 'uno', 'two': 'dos' }
        self.assertEqual('uno', babel_fish['one']) #one = uno
        self.assertEqual('dos', babel_fish['two']) #two = dos

    def test_changing_dictionaries(self):
        babel_fish = { 'one': 'uno', 'two': 'dos' }
        babel_fish['one'] = 'eins'

        expected = { 'two': 'dos', 'one': 'eins',} #one equals 'eins' in lne #36
        self.assertDictEqual(expected, babel_fish)

    def test_dictionary_is_unordered(self):
        dict1 = { 'one': 'uno', 'two': 'dos' }
        dict2 = { 'two': 'dos', 'one': 'eins' }

        self.assertEqual(False, dict1 == dict2) #dict1 does not equal dict2 therefore the answer is false
    ...
    def test_making_a_dictionary_from_a_sequence_of_keys(self):
        ...
        self.assertEqual(42, cards['green elf']) #42 is the number of cards 
